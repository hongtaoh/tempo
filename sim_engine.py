...
def very_irregular_distribution(
    ...
    """
    Generate highly irregular, non-normal samples for a given distribution family and state.

    Six distribution families (dist_type 0-5) are available. Each is parameterized
    relative to the biomarker's pre-event or post-event mean and std, so any
    (mean, std > 0) is valid regardless of the chosen family. The family is randomly
    assigned per biomarker per dataset at the call site.

    Args:
        dist_type: Integer 0-5 selecting the distribution family.
            0: Triangular + Normal + Exponential
            1: Pareto + Uniform + Logistic
            2: Beta + Exponential(signed) + Normal(spike)
            3: Gamma + Weibull + Normal(±σ)
            4: Heavy-tailed Cauchy (single component)
    ...
    if dist_type == 0:
        ...
        # Beta + Exponential (signed) + Normal with spike
        ...
        # Gamma + Weibull + Normal with ±σ spike
...
def generate_measurements_ebm(
    ...
    """
    Generate measurements using the EBM binary-switch model.

    Each biomarker is either "affected" (disease_stage >= event_time) or
    "nonaffected", regardless of whether k_j is ordinal or continuous.
    Supports optional per-participant event-time noise for xiNearNormalWithNoise
    experiments.
    ...
    Args:
        ...
        all_kjs: Array of disease stages for all participants (ordinal or continuous)
        ...
        noise_std_parameter: Std scale for per-participant event-time noise (xiNearNormalWithNoise only)
    ...
    if "xnjNonNormal" in experiment_name:
        ...
        # Randomly assign one of 6 distribution families per biomarker per dataset
        dist_type_dict = {b: int(rng.integers(0, 6)) for b in shuffled_biomarkers}
...
def generate_data(
    ...
    if "kjOrdinal" in experiment_name:
        ...
        data = generate_measurements_ebm(
            ...
            all_diseased, keep_all_cols, noise_std_parameter=noise_std_parameter, rng=rng)
        ...
        # Generate measurements: sigmoid progression or EBM binary-switch model
