...
class UnifiedTransformer(nn.Module):
    ...
    Both branches are always active regardless of biomarker count:

    Ranking branch:
      - patient_encoder pools each biomarker's values across patients
      - biomarker_transformer attends over biomarkers to produce ranking scores

    Staging branch:
      1. abnormality_detector (MLP, 3 inputs: bio_val, diseased, bio_score) gives
         per-biomarker abnormality probability in [0, 1], informed by ranking scores
      2. stage_transformer (patient attention) operates on the (n_samples x n_bio)
         abnormality prob matrix — patients contextualise each other's profiles
      3. stage_head predicts per-patient stage

    Compared to SimpleTransformer: cleaner input to patient attention (probs, not raw values).
    Compared to ConnectedTransformer: richer aggregation (attention, not sum+MLP).
    """
    def __init__(self, n_biomarkers=10, max_stage=10):
        ...
        # === STAGING BRANCH: Abnormality detector + Patient attention ===

        # Per-biomarker detector: inputs are (bio_val, diseased, bio_score)
        ...
        # Patient transformer on the n_bio-dim abnormality prob vectors
        self.stage_encoder = nn.Sequential(
            nn.Linear(n_biomarkers, d * 2),
            nn.LayerNorm(d * 2),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
    ...
    def forward_stage(self, x, biomarker_scores):
        ...
        # Step 1: per-biomarker abnormality detection
        abnormality_probs = []
        for i in range(n_bio):
        ...
        # (batch, n_samples, n_bio) — clean [0,1] probabilities
        abnormality_probs = torch.stack(abnormality_probs, dim=-1)

        # Step 2: patient-attention transformer on abnormality probs
        encoded = self.stage_encoder(abnormality_probs)   # (batch, n_samples, d*2)
        encoded = self.stage_transformer(encoded)          # patients attend to each other
        out = self.stage_head(encoded).squeeze(-1)         # (batch, n_samples)
...
def create_model(n_biomarkers, max_stage):
    print(f"Using UnifiedTransformer (n_bio={n_biomarkers})")
    return UnifiedTransformer(n_biomarkers, max_stage)
...
def main():
    ...
    # Pick epoch budget based on data dimensionality
    if 'highdim' in CONFIG['train_data_dir']:
        CONFIG['epochs'] = CONFIG.get('epochs_highdim', CONFIG.get('epochs', 75))
    else:
        CONFIG['epochs'] = CONFIG.get('epochs_lowdim', CONFIG.get('epochs', 25))
    ...
    print(f"="*60)
    print(f"Unified Transformer Training")
    print(f"="*60)
    ...
    mode = "highdim" if "highdim" in CONFIG.get('train_data_dir', '') else "lowdim"
    per_exp_file = os.path.join("logs", f"per_exp_{mode}.txt")
    ...
    for exp in target_exps:
        ...
        timing_line = f"  {exp}: {h:02d}:{m:02d}:{s:02d}"
        print(timing_line)
        with open(per_exp_file, "a") as tf:
            tf.write(timing_line + "\n")
