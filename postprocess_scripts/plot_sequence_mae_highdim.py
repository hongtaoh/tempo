...
# 1. Load Data
csv_path = os.path.join(ROOT_DIR, "results", "100bio", "seq_mae_pivot.csv")
...
plt.title('Generalization Matrix: Sequence MAE Across Conditions (High Dimensional)', fontsize=14, pad=20)
...
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, 'sequence_mae_highdim.pdf'), dpi=300)
plt.savefig(os.path.join(PLOTS_DIR, 'sequence_mae_highdim.png'), dpi=300)
plt.show()
